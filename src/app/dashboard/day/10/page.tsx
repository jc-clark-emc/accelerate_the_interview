...
import { DAYS, NETWORKING_STATUSES } from "@/lib/constants";
import {
  ArrowLeft,
  CheckCircle,
  Plus,
...
export default function Day10Page() {
  ...
  const dayInfo = DAYS[9]; // Day 10

  // Load first 5 jobs with contacts
  useEffect(() => {
    async function loadData() {
      try {
        ...
        if (res.ok) {
          ...
          // Get first 5 jobs that have been applied to
  ...
  // Calculate progress
  const totalContacts = jobs.reduce((sum, j) => sum + j.contacts.length, 0);
  ...
  const handleComplete = async () => {
    ...
    try {
      await fetch("/api/day-progress", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ dayNumber: 10, status: "COMPLETED" }),
      });
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
        <div className="flex items-start gap-2">
          ...
          <div className="text-sm text-blue-800">
            <p className="font-medium">For each company, find:</p>
            <ul className="list-disc list-inside mt-1 space-y-1">
              <li>3 Hiring Team (recruiter, hiring manager, HR)</li>
              <li>3 Potential Coworkers (people in similar roles)</li>
            </ul>
            <p className="mt-2">
              Use LinkedIn to search "[Company] [Role]" to find the right people.
            </p>
          </div>
        </div>
      </div>
